...
import styles from './AddressShop.module.scss';
...
function AddressUser() {
    ...
    const handleOk = async () => {
        try {
            ...
            await shopServices.addShopAddress({ address: addressContent, area: areaContent });
            setIsModalVisible(false);
            success1();
            ...
            if (response.status === 200) {
                setAddresses(response.data.areas);
            }
        } catch (error) {
            console.error(error);
            errorMessage1();
    ...
    const handleRemove = async (id) => {
        try {
            const response = await shopServices.deleteShopAddress(id);
    ...
    return (
        <div className={cx('wrapper')}>
            ...
            <div className={cx('header')}>
                <h1>Danh sách địa chỉ của cửa hàng</h1>
                <ConfigProvider
                    theme={{
                        components: {
                            Button: {
